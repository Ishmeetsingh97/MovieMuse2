...
public class MovieDetailsFragment extends BaseFragment implements
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        objMovie = Movie.fromParcelable(getArguments().getParcelable(BundleKeys.MOVIE));
        if (objMovie == null) {
        ...
        AppUtil.setupToolbar(getActivity(), mToolbar, navIcon, objMovie.getTitle());
    ...
    public void onStart() {
        super.onStart();
        getDataBus().post(new LoadMovie(objMovie.getId()));
    }
    ...
    public boolean onMenuItemClick(MenuItem item) {
        switch (item.getItemId()) {
            case R.id.share_trailer:
                Video firstTrailer = Movie.getTrailers(objMovie).get(0);
                String subject = objMovie.getTitle() + " - " + firstTrailer.getName();
    ...
    public void onClick(View v) {
        if (v.getId() == R.id.favorite && objMovie != null) {
            Movie movieCopy = AppUtil.copy(objMovie, Movie.class);
            if (movieCopy != null) {
                movieCopy.setFavorite(!movieCopy.isFavorite());
                getDataBus().post(new UpdateMEvent(movieCopy));
            }
    ...
    private void updateMovieDetails() {
        ...
        mToolbar.setTitle(objMovie.getTitle());
        ...
        RequestCreator backdropRequest = picasso
                .load(TMDbUtil.buildBackdropUrl(objMovie.getBackdropPath(), backdropWidth))
        ...
        RequestCreator posterRequest = picasso
                .load(TMDbUtil.buildPosterUrl(objMovie.getPosterPath(), posterWidth))
        ...
        mTitle.setText(objMovie.getTitle());

        Calendar calendar = Calendar.getInstance();
        calendar.setTime(objMovie.getReleaseDate());
        mReleaseDate.setText(String.valueOf(calendar.get(Calendar.YEAR)));

        mRating.setText(String.format("%1$2.1f", objMovie.getRating()));
        mSynopsis.setText(objMovie.getSynopsis());
    }

    @Subscribe
    public void onMovieLoadedEvent(MovieLoadedEvent event) {
        objMovie = event.movie;

        updateFavoriteBtn(mInitialMovieLoaded ? 0 : mAnimActivityStartDelay);
        List<Review> reviews = objMovie.getReviews();
        List<Video> trailers = Movie.getTrailers(objMovie);
        if (BuildConfig.DEBUG) {
            Log.d(TAG, String.format("%1$s: fav=%2$s reviews=%3$d trailers=%4$d",
                    objMovie.getTitle(), String.valueOf(objMovie.isFavorite()),
    ...
    private void updateFavoriteBtn(final int animStartDelay) {
        mFavoriteBtn.setImageDrawable(objMovie.isFavorite() ? mStarFilled : mStarOutline);
